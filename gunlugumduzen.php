...
<center><div class="gunluk">
    ...
    <input type="submit" class="kaydet"  name="kaydet" value=" Güncelle ">
</div></center>
</form>
...
//    $gunlukid=$_SESSION["gunluk_id"];

//     if ($gunlukid==$row['gunluk_id'] ){
    $x = $_GET['gid'];
    $b = $_POST['baslik'];
    $i = $_POST['icerik'];
    $t = $_POST['tarih'];
    $db = new PDO("mysql:host=localhost;dbname=mertmaki_gunlukizler", 'root', ''); // bağlantı oluşturuluyor
    $guncel = $db->prepare("UPDATE `gunluk` SET `baslik`=?, `icerik`=?, `tarih`=?  WHERE gunluk_id=$x");
    $guncel->execute([$b, $i, $t ]);
